...
public class MinStack<T> {
    ...
    private void siftDown() {
        ...
        while (children.length != 0) {
            ...
            int indexWithMinElement = getMinElementOfChildren(children);

            Element<T> rootElement = (Element<T>) array[rootIndex];

            if (rootElement.getPriority() > ((Element<T>) array[indexWithMinElement]).getPriority()) {
                // значения ребенка переносим вверх
                array[rootIndex] = array[indexWithMinElement];
                array[indexWithMinElement] = rootElement;

                rootIndex = indexWithMinElement;
    ...
    private int getMinElementOfChildren(int[] childrenIndexes) {
